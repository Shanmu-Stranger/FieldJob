app.controller("debriefController", function ($scope, $state, $rootScope, $window, $timeout, $filter, cloudService, $mdDialog, valueService, localService, Upload, constantService) {
    ...
    $scope.initializeDebrief = function () {
        ...
        else if ($rootScope.online)
        {
            ...
            if($scope.installBaseObject==undefined)
            {
                ...
                cloudService.getInstallBaseListCloud(form,function(response)
                {
            ...
            // $scope.chargeMethodArray = cloudService.getChargeMethodCloud();
            //
            var date=new Date();
            date.setMonth(date.getMonth()-1);
            var endDate =new Date();
            ...
            //
            // $scope.overTimeArray = cloudService.getOverTimeListCloud();
            //
            // $scope.shiftCodeArray = cloudService.getShiftCodeListCloud();
            //
            // $scope.chargeTypeArray = cloudService.getChargeTypeCloud();
            //
            // $scope.fieldJobArray = cloudService.getFieldJobNameCloud();
    ...
    $scope.setDropDownValues = function () {
        ...
        $scope.expenseDefault = {
            ...
            expenseType: {
                title: "Expense Type",
                values: [{"id": 7, "value": "Airfare"},
                {"id": 8, "value": "Per Diem"},
                {"id": 9, "value": "Meals- Breakfast"},
                {"id": 10, "value": "Meals- Lunch"},
                {"id": 11, "value": "Meals- Dinner"},
                {"id": 12, "value": "Parking"},
                {"id": 13, "value": "Lodging"},
                {"id": 14, "value": "Rental Car"},
                {"id": 15, "value": "Mileage"},
                {"id": 16, "value": "Tolls"}]
            },
            amount: {
            ...
            currency: {
                title: "Currency",
                values:[],
                //     [{"id": "1", "value": "US Dollar (USD)"},
                // {"id": "2", "value": "Chinese Yuan (CNY)"},
                // {"id": "3", "value": "British Pound (GBP)"},
                // {"id": "4", "value": "Euro (EUR)"}]
            },
            chargeMethod: {
                title: "Charge Method",
        ...
        if ($rootScope.local) {
            ...
            $scope.timeDefault.chargeType.values = $scope.chargeTypeArray;
            $scope.timeDefault.chargeMethod.values = $scope.chargeMethodArray;
            $scope.timeDefault.fieldJobName.values = $scope.fieldJobArray;
            $scope.timeDefault.workType.values = valueService.getWorkType();
            //$scope.timeDefault.item.values = valueService.getItem();
            $scope.expenseDefault.currency.values = valueService.getCurrency();

            $scope.expenseDefault.chargeMethod.values = $scope.chargeMethodArray;

        }
        //else if ($rootScope.online)
        //{
        //    constantService.setResourceId("5");

        //    $scope.timeDefault.shiftCode.values = valueService.getShiftCode();
        //    $scope.timeDefault.timeCode.values = valueService.getOverTime();
        //    $scope.timeDefault.chargeType.values = valueService.getChargeType();
        //    $scope.timeDefault.chargeMethod.values = valueService.getChargeMethod();
        //    $scope.timeDefault.fieldJobName.values = valueService.getFieldJob();
        //    $scope.timeDefault.workType.values = valueService.getWorkType();
        //    //$scope.timeDefault.item.values = valueService.getItem();
        //    $scope.expenseDefault.currency.values = valueService.getCurrency();
        ...
        //}
            ...
            cloudService.getShiftCodeListCloud(function (response) {

                $scope.timeDefault.shiftCode.values = [];

                if (response != undefined && response.ShiftCode != undefined && response.ShiftCode.length > 0) {

                    angular.forEach(response.ShiftCode, function (key, value) {

                        if (key.TaskNumber == $scope.taskId) {

                            $scope.timeDefault.shiftCode.values.push(key);
                        }
                    });

                    $scope.shiftCodeArray = $scope.timeDefault.shiftCode.values;
                    $scope.setDefaultValues();
                }
            });

            cloudService.getOverTimeListCloud(function (response1) {

                $scope.timeDefault.timeCode.values = [];

                if (response1 != undefined && response1.OverTImeShiftCode != undefined && response1.OverTImeShiftCode.length > 0) {

                    angular.forEach(response1.OverTImeShiftCode, function (key, value) {

                        if (key.Task == $scope.taskId) {

                            $scope.timeDefault.timeCode.values.push(key);
                        }
                    });

                    $scope.overTimeArray = $scope.timeDefault.timeCode.values;
                    $scope.setDefaultValues();
                }
            });

            cloudService.getChargeTypeCloud(function (response) {

                $scope.timeDefault.chargeType.values = response.Charge_Type;
                $scope.setDefaultValues()
            });
            cloudService.getWorkTypeCloud(function (response) {

                $scope.timeDefault.workType.values = response.Charge_Method;
                $scope.setDefaultValues()
            });
            // cloudService.getItemCloud(function (response) {
            //
            //     $scope.timeDefault.chargeType.values = response.Charge_Method;
            //
            // });

            cloudService.getCurrencyCloud(function (response) {

                $scope.expenseDefault.currency.values = response.Charge_Method;
                $scope.setDefaultValues()
            });

            cloudService.getChargeMethodCloud(function (response) {

                $scope.timeDefault.chargeMethod.values = response.Charge_Method;

                if ($scope.timeArray.length > 0) {

                    angular.forEach(response.Charge_Method, function (key, value) {
                ...
                $scope.expenseDefault.chargeMethod.values = response.Charge_Method;
                $scope.timeDefault.chargeMethod.valuesDeputation = response.Charge_Method;
                $scope.timeDefault.chargeMethod.valuesNightShift = response.Charge_Method;
                $scope.timeDefault.chargeMethod.valuesNormal = response.Charge_Method;
                $scope.timeDefault.chargeMethod.valuesTravel = response.Charge_Method;
                $scope.setDefaultValues();
            });

            cloudService.getFieldJobNameCloud(function (response) {

                $scope.timeDefault.fieldJobName.values = [];

                angular.forEach(response.TaskName, function (job, value) {

                    if (job.ID == $scope.taskId) {
                        $scope.timeDefault.fieldJobName.values.push(job);
                        $scope.setDefaultValues()
                    }
                });
            });
        ...
        $scope.noteDefault = {
            noteType: {
                title: "Note Type",
                values: [{"id": 131, "value": "Action Taken"},
                {"id": 129, "value": "Customer Site Information"},
                {"id": 126, "value": "General Information"},
                {"id": 130, "value": "Inventory/Material Request"},
                {"id": 128, "value": "Invoice Note"},
                {"id": 127, "value": "Problem Description"}]
            },
            noteDate: {
                title: "Note Date"
            },
    ...
    $scope.setDefaultValues = function () {
        ...
        if ($scope.timeArray != null && $scope.timeArray.length > 0) {
            ...
            angular.forEach($scope.timeArray, function (item) {
                ...
                console.log("*************" + item.Charge_Type_Id);
    ...
    {
        timeObject.Field_Job_Name_Id = timeObject.Field_Job_Name.ID;
    }
...
;
    ...
    //ms-appdata:///local/Report_1112.pdf
...
//                 doc1.save("Report_" + $scope.summary.taskObject.Task_Number + ".pdf");
    ...
    if ($rootScope.local) {
        ...
        valueService.saveBase64File(filePath, "Report_" + $scope.summary.taskObject.Task_Number + ".pdf", $scope.value, "application/pdf");
